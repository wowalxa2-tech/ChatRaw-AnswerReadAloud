...
测试需要一个单独运行的、已应用兼容补丁的 ChatRaw 实例，并已安装本插件。不要在真实聊天数据所在页面运行浏览器测试；测试会替换页面消息并启停插件。
...
需要自己的百炼 API Key，在插件设置点击「保存并试听」，再朗读实际回答。确认声音、发音、首段等待、段间停顿和云端计费。模拟测试不能代替此项，v1.0.0 尚未用真实密钥验收。
